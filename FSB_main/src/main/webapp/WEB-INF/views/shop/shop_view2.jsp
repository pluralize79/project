...
/* 	function search(mode){
		window.open("search_member.do?mode="+mode, "", "width=640, height=400")
	} */
	function loginCheck(){
		if (f.id.value==""){
			alert("아이디를 입력해 주세요!!")
...
<c:if test="${not empty getViewReview}">         
      ...
      <div class="row justify-content-center">
         <div class="col-4 bg-white border-bottom py-3 pb-2">
            <font size="4.5"><b>리뷰 (${count})</b></font> <a href="shop_insertReview.do">쓰기</a>
         </div>
         <div class="col-5 bg-white border-bottom py-3 pb-2" align="right">
            <a href="#"><button class="btn btn-outline-dark btn-sm" type="button">전체보기</button></a>            
         </div>
...
<c:if test="${not empty getViewReview}">	
		...
		<c:if test="${dto.sr_img1 ne null}">
         ...
         <div class="col-1 bg-white border-bottom py-3 pb-2" height="300" align="right">
            ...
            <a href="shop_updateReview.do?sr_num=${dto.sr_num}"><button class="btn btn-outline-dark" type="button">&nbsp;수정&nbsp;</button></a>   <!-- 로그인시 활성화 -->   
         </div>
         <div class="col-1 bg-white border-bottom py-3" height="300" align="left">
            <h6>${dto.mem_nickname}</h6>
            <h6>${dto.sr_regdate}</h6>   
            <br><br><br><br><br><br><br><br><br>
            <a href="javascript:checkDel('${dto.sr_num}','${dto.sr_img1}','${dto.sr_img2}','${dto.sr_img3}','${dto.sr_img4}')"><button class="btn btn-outline-dark" type="button">&nbsp;삭제&nbsp;</button></a>   <!-- 로그인시 활성화 -->         
         </div>
         </c:if>
         <!-- 상품 리뷰에 사진이 없을때 -->
		 <c:if test="${dto.sr_img1 eq null}">
...
<form name="f2" action="shop_deleteReview.do" method="post">
