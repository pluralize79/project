			...
			<p>
				<div class="row row-cols-1" style="overflow: scroll; height: 800px;">
							...
							<c:forEach var="dto" items="${listBoard}">
								<tr>
									...
									<td>
										...
										<a href="admin_board_view.do?view=anony&board_num=${dto.board_num}&sort=all">${dto.board_title}</a> 
					...
					<form name="f2" action="admin_board_delete.do" method="post">
						...
						<input type="hidden" name="view" value="anony">
					...
					<nav aria-label="Page navigation example">
						...
						<c:if test="${count>0}">
					   		<c:if test="${startPage > pageBlock}">
					   			 <li class="page-item">
					     		 	<a class="page-link" href="admin_board_list.do?view=anony&pageNum=${startPage-pageBlock}" aria-label="Previous">
					   		...
					   		<c:forEach var="i" begin="${startPage}" end="${endPage}">
					   			 <li class="page-item"><a class="page-link" href="admin_board_list.do?view=anony&pageNum=${i}">${i}</a></li>
					    	</c:forEach>
						   	<c:if test="${endPage < pageCount}">
					   			<li class="page-item">
					      			<a class="page-link" href="admin_board_list.do?view=anony&pageNum=${startPage+pageBlock}" aria-label="Next">
