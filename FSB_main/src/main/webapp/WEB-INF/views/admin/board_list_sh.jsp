			...
			<p>
				<div class="row row-cols-1" style="overflow: scroll; height: 800px;">
					...
					<form name="f2" action="admin_board_delete.do" method="post">
						...
						<input type="hidden" name="view" value="secondhand">
					...
					<nav aria-label="Page navigation example">
						...
						<c:if test="${count>0}">
					   		<c:if test="${startPage > pageBlock}">
					   			 <li class="page-item">
					     		 	<a class="page-link" href="admin_board_list.do?view=secondhand&pageNum=${startPage-pageBlock}" aria-label="Previous">
					   		...
					   		<c:forEach var="i" begin="${startPage}" end="${endPage}">
					   			 <li class="page-item"><a class="page-link" href="admin_board_list.do?view=secondhand&pageNum=${i}">${i}</a></li>
					    	</c:forEach>
						   	<c:if test="${endPage < pageCount}">
					   			<li class="page-item">
					      			<a class="page-link" href="admin_board_list.do?view=secondhand&pageNum=${startPage+pageBlock}" aria-label="Next">
